...
- 数据集: UniRef50 转换后的 SMILES (292K 序列)
- 模型: 大型对比学习模型
- 使用真实的肽段 SMILES 数据
"""
...
from data_loader_lazy import create_dataloaders


def train_epoch(model, train_loader, optimizer, aug, device, epoch, accumulation_steps=4):
...
def main():
    ...
    print("=" * 60)
    print(f"v0.5.0 - UniRef50 SMILES 大规模预训练 (GPU {args.gpu})")
    print("=" * 60)
    ...
    config = {
        ...
        'batch_size': 16,  # 较小batch size，序列很长
        'accumulation_steps': 4,  # 有效batch size = 64
        'hidden_dim': 512,
        'num_layers': 8,
        'num_heads': 8,
        'dropout': 0.1,
    ...
    for epoch in range(config['num_epochs']):
        ...
        if loss < best_loss and not np.isnan(loss):
            ...
            torch.save({
                ...
                'loss': loss,
                'config': config
            }, f'test_results/best_v5_uniref_gpu{args.gpu}.pt')
        ...
        if (epoch + 1) % 10 == 0:
            torch.save({
                ...
                'loss': loss,
                'config': config
            }, f'test_results/checkpoint_v5_uniref_gpu{args.gpu}_epoch_{epoch+1}.pt')
    ...
    if not np.isnan(loss):
        torch.save({
            ...
            'losses': losses,
            'final_loss': loss
        }, f'test_results/final_v5_uniref_gpu{args.gpu}.pt')

        print("\n✓ 预训练完成!")
        print(f"  最佳损失: {best_loss:.4f}")
        print(f"  最终损失: {loss:.4f}")
        print(f"  编码器已保存到: test_results/best_v5_uniref_gpu{args.gpu}.pt")
    else:
        print("\n✗ 训练失败（NaN损失）")
