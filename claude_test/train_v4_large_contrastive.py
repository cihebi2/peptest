...
"""
v0.4.0 - 更大规模对比学习预训练
- 使用更大的模型 (hidden_dim=512, num_layers=8)
- 更长的训练 (150 epochs)
- 梯度累积以支持更大的有效batch size
GPU: 1 或 4
"""
...
def main():
    ...
    config = {
        ...
        'accumulation_steps': 4,  # 有效batch size = 16*4 = 64
        'hidden_dim': 512,  # 更大
        'num_layers': 8,  # 更深
        'num_heads': 8,
        'dropout': 0.1,
        'projection_dim': 256,  # 更大
        'temperature': 0.5,
        'learning_rate': 3e-5,  # 更小的学习率
        'weight_decay': 1e-5,
        'num_epochs': 150,  # 更长的训练
        'max_samples': None,
    ...
    for epoch in range(config['num_epochs']):
        ...
        if loss < best_loss and not np.isnan(loss):
            ...
            torch.save({
                ...
                'loss': loss,
                'config': config
            }, 'test_results/best_v4_large_contrastive.pt')
        ...
        if (epoch + 1) % 20 == 0:
            torch.save({
                ...
                'loss': loss,
                'config': config
            }, f'test_results/checkpoint_v4_large_epoch_{epoch+1}.pt')
    ...
    if not np.isnan(loss):
        torch.save({
            ...
            'losses': losses,
            'final_loss': loss
        }, 'test_results/final_v4_large_contrastive.pt')

        print("\n✓ 预训练完成!")
        print(f"  最佳损失: {best_loss:.4f}")
        print(f"  最终损失: {loss:.4f}")
        print(f"  编码器已保存到: test_results/best_v4_large_contrastive.pt")
    else:
        print("\n✗ 训练失败（NaN损失）")
